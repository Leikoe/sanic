...
<https://developer.apple.com/documentation/metal/mtlblitcommandencoder/waitforfence(_:)>
...
You can synchronize memory operations of a blit pass that access resources with an [MTLFence](https://developer.apple.com/documentation/metal/mtlfence). This method instructs the GPU to wait until another pass updates `fence` before running the blit pass. The fence indicates when the pass can access those resources without a race condition.
