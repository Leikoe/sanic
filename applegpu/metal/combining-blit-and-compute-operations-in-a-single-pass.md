...
*Sample Code · iOS 26.0, iPadOS 26.0, Mac Catalyst 26.0, macOS 26.0, tvOS 26.0, Xcode 26.3*

<https://developer.apple.com/documentation/metal/combining-blit-and-compute-operations-in-a-single-pass>
...
This sample builds on Metal 4 fundamentals in [Drawing a triangle with Metal 4](https://developer.apple.com/documentation/metal/drawing-a-triangle-with-metal-4), including command buffers, allocators, residency sets, and submitting work to the GPU.
...
An [MTL4ComputeCommandEncoder](https://developer.apple.com/documentation/metal/mtl4computecommandencoder) instance combines blit, dispatch, and acceleration structure commands into a single pass, which reduces your app’s overhead from creating multiple encoders and encoding separate passes. The app increases its GPU utilization by running independent blit commands that concurrently write to memory regions that don’t overlap. However, it prevents the blit and dispatch stage memory operations from running at the same time, by synchronizing them with an intrapass barrier.

![image](https://docs-assets.developer.apple.com/published/96b6bb182776b7b6d12ff9f7caf8fed8/combining-blit-and-compute-operations-in-a-single-pass%402x.png)
...
The renderer’s initializer creates the fundamental Metal instances it needs: a command queue, a reusable command buffer, and the default library. For more information on how these work, see [Drawing a triangle with Metal 4](https://developer.apple.com/documentation/metal/drawing-a-triangle-with-metal-4).
...
The `loadImageToTexture:` method creates an instance of [MTLTexture](https://developer.apple.com/documentation/metal/mtltexture) with a pixel format that stores four color channels: blue, green, red, and alpha. Each channel uses an 8-bit unnormalized value, where `0` maps to `0.0` and `255` maps to `1.0`:
...
After creating the texture, the method copies the image data into it with its [replace(region:mipmapLevel:withBytes:bytesPerRow:)](https://developer.apple.com/documentation/metal/mtltexture/replace(region:mipmaplevel:withbytes:bytesperrow:)) method:
...
The app creates two additional textures to store the composite images, by configuring an instance of [MTLTextureDescriptor](https://developer.apple.com/documentation/metal/mtltexturedescriptor) with dimensions that fit both source images. The first texture stores the composite color texture that the compute pass creates from the background and chyron textures. The second texture stores the grayscale equivalent of the composite color texture.
...
The `createArgumentTable` method configures an instance of [MTL4ArgumentTableDescriptor](https://developer.apple.com/documentation/metal/mtl4argumenttabledescriptor) with the largest number of buffer and texture bindings the app ever needs at one time:
...
The renderer compiles a render pipeline at launch time by creating an instance of [MTL4Compiler](https://developer.apple.com/documentation/metal/mtl4compiler) with a default configuration:
...
The `createComputePipeline` method creates an [MTL4LibraryFunctionDescriptor](https://developer.apple.com/documentation/metal/mtl4libraryfunctiondescriptor) that refers to the `convertToGrayscale` kernel in the default library:
...
For more information about access conflicts and synchronization, see [Resource synchronization](https://developer.apple.com/documentation/metal/resource-synchronization) and [Synchronizing stages within a pass](https://developer.apple.com/documentation/metal/synchronizing-stages-within-a-pass).
...
The renderer resolves this access conflict by encoding a consumer barrier between the [dispatch](https://developer.apple.com/documentation/metal/mtlstages/dispatch) and [fragment](https://developer.apple.com/documentation/metal/mtlstages/fragment) stages:
...
The `afterQueueStages` parameter refers to stages in earlier passes, whereas the `beforeStages` parameter refers to stages in the current and later passes. The app passes [fragment](https://developer.apple.com/documentation/metal/mtlstages/fragment) to `beforeStages` because only the fragment stage accesses the texture, which means the GPU can run the vertex stage ([vertex](https://developer.apple.com/documentation/metal/mtlstages/vertex)) before the compute pass finishes.
...
> **Note:**
>  You can modify the entries in an [MTL4ArgumentTable](https://developer.apple.com/documentation/metal/mtl4argumenttable) instance after passing it to an encoder’s [setArgumentTable(_:)](https://developer.apple.com/documentation/metal/mtl4computecommandencoder/setargumenttable(_:)) method because Metal evaluates the argument table each time you call a command-encoding method.
