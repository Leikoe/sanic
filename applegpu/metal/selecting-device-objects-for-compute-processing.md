...
*Sample Code · macOS 10.13, Xcode 12.4*
...
![image](https://docs-assets.developer.apple.com/published/61492c1c60f708912c1f36cb388a1796/1-single-device-executation.png)
...
![image](https://docs-assets.developer.apple.com/published/ae88bbf6e567e13477a82d670696f9d0/2-multi-device-executation.png)
...
This sample implements many techniques described in [Selecting device objects for graphics rendering](https://developer.apple.com/documentation/metal/selecting-device-objects-for-graphics-rendering). For information about handling external GPU notifications, see the following sections from that sample:
...
When the sample runs on a single device, the view controller executes the simulation and renderer work serially. For each frame, they both encode commands to the same [MTLCommandBuffer](https://developer.apple.com/documentation/metal/mtlcommandbuffer) and they both share the positions of the N-body particles via the same [MTLBuffer](https://developer.apple.com/documentation/metal/mtlbuffer).

![image](https://docs-assets.developer.apple.com/published/239dfde287be75deb3e951a6b772b82d/3-single-device-buffer.png)

When the sample runs on multiple devices, the view controller assigns the simulation work to one device and the renderer work to another. The simulation executes repeatedly in a loop on a separate simulation thread. For each iteration, it updates the positions of the N-body particles and blits this data to a new [MTLBuffer](https://developer.apple.com/documentation/metal/mtlbuffer) backed by system memory. The sample passes this system memory backing to the view controller, which then passes it along to the renderer. The renderer executes repeatedly in a loop on the main thread. For each iteration, it creates a new [MTLBuffer](https://developer.apple.com/documentation/metal/mtlbuffer), backed by the same system memory populated by the simulation thread, and renders the N-body particles based on the latest available position data.

> **Note:**
> An [MTLBuffer](https://developer.apple.com/documentation/metal/mtlbuffer) can’t be directly transferred between different devices; its data needs to be transferred via system memory.

![image](https://docs-assets.developer.apple.com/published/18c1da1a7bf62179fe106b3b1eaf7585/4-multi-device-buffer.png)

### Allocate system memory for a buffer

The sample calls the `vm_allocate` function to allocate a page-aligned buffer, `updateAddress`, backed by system memory. The sample then calls the [makeBuffer(bytesNoCopy:length:options:deallocator:)](https://developer.apple.com/documentation/metal/mtldevice/makebuffer(bytesnocopy:length:options:deallocator:)) method to create a new [MTLBuffer](https://developer.apple.com/documentation/metal/mtlbuffer), `_updateBuffer`, backed by the same system memory used for the previous buffer.
