...
fn main() {
    ...
    let scores = matmul(input("Q", &[s2, dk2]), input("K", &[t2, dk2]), dk2);
    let scaled = map(MapOp::Mul, vec![scores, konst(0.125)]);
    let masked = map(MapOp::Add, vec![scaled, causal_mask(s2, t2)]);
    let causal = matmul(softmax(masked, t2), input("V", &[t2, dv2]), t2);
    ...
    let o1 = reduce(
        map(MapOp::Mul, vec![w4.clone(), input("V1", &[k4])]),
        k4,
        BinOp::Monoid(Monoid::Add),
    );
    let o2 = reduce(
        map(MapOp::Mul, vec![w4, input("V2", &[k4])]),
        k4,
        BinOp::Monoid(Monoid::Add),
    );
