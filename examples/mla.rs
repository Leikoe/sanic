...
//! single-head attention. The structure map makes the equivalence explicit:
//! standard MLA shows two inner contractions (dc, dh); the absorbed form
//! collapses them into one (dc only).
//!
//!     cargo run --example mla

use sanic::emit_rust::rust_kernel;
use sanic::ir::*;
use sanic::{Parallelism, analyze_all, derive};

fn main() {
    let (k, dc, dh, de, sq) = (axis("k"), axis("dc"), axis("dh"), axis("de"), axis("sq"));
    ...
    // built through two up-projections. The deriver only sees the streaming
    // axis k; everything else is a per-k sub-expression treated as a leaf.
    let car_std = derive(&out_std, k).expect("k is foldable");
    ...
    let car_abs = derive(&out_abs, k).expect("k is foldable");
    println!("derived carrier for k:\n{}\n", car_abs.render());
    ...
    for (name, car) in [
        ...
        println!("  {name:<10}  {} slots  rules {:?}", car.slots, car.rules);
    }

    // combine and project are byte-identical across all three.
    assert_eq!(
        format!("{:?}", sha_car.combine),
        format!("{:?}", car_std.combine)
    );
    assert_eq!(
        format!("{:?}", sha_car.combine),
        format!("{:?}", car_abs.combine)
    );
    assert_eq!(
        format!("{:?}", sha_car.project),
        format!("{:?}", car_std.project)
    );
    assert_eq!(
        format!("{:?}", sha_car.project),
        format!("{:?}", car_abs.project)
    );
    ...
    // Each streaming element: (score, value) — same interface as attention.
