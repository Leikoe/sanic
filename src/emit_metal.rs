...
//!
//! [`emit_fused_metal_with`] emits one fused kernel; [`emit_schedule_metal_on`]
//! emits a whole schedule (every kernel + dispatch bookkeeping) for a
//! multi-kernel host to run. Emits `float` (Apple GPUs have no f64), so
//! verification uses an f32 tolerance. `tests/metal.rs` compiles the MSL,
...
/// (in `[[buffer(0..)]]` order), their declared storage dtypes, and the
/// dispatch size.
...
/// the fold's result as an input leaf plus other buffers, rendered IN the
/// kernel after the projection — the projection lands in a register,
/// `Gen::local_inputs` resolves the read, one dispatch instead of a fold
/// plus an in-place map.
...
/// axis splits across simdgroups and/or lanes, an optional lane-distributed
