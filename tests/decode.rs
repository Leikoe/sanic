...
fn decode_step(t: Axis, dm: Axis, dk: Axis, dv: Axis, v: Axis) -> Schedule {
    ...
    let new_k = linear_vector(x.clone(), input("Wk", [dk, dm])); // [dk]
    let new_v = linear_vector(x.clone(), input("Wv", [dv, dm])); // [dv]
    let q = linear_vector(x, input("Wq", [dk, dm])); // [dk]
    ...
    let logits = linear_vector(out, input("Wl", [v, dv])); // [v]
...
fn prefill_logits(s: Axis, t2: Axis, dm: Axis, dk: Axis, dv: Axis, v: Axis, env: &Env) -> Value {
    ...
    let q = matmul(x, transpose(input("Wq", [dk, dm]), 0usize, 1usize)); // [s, dk]
    let k = matmul(xt.clone(), transpose(input("Wk", [dk, dm]), 0usize, 1usize)); // [t2, dk]
    let vv = matmul(xt, transpose(input("Wv", [dv, dm]), 0usize, 1usize)); // [t2, dv]
    let scale = konst(1.0 / (dk.extent() as f64).sqrt());
    ...
    let logits = matmul(out, transpose(input("Wl", [v, dv]), 0usize, 1usize)); // [s, v]
    eval(&logits, env)
}
...
fn incremental_decode_equals_prefill() {
    ...
    let k_ref = eval(
        &matmul(
            rename(input("X", [s, dm]), 0usize, t),
            transpose(input("Wk", [dk, dm]), 0usize, 1usize),
        ),
        &prefill_env,
    );
